...
export class Viriya {
    ...
    async removeActiveFileFromViriya(): Promise<void> {
        const { vault, workspace } = this.plugin.app;
        ...
        const base: unknown[] = Array.isArray(cache?.frontmatter?.['base'])
            ? cache.frontmatter['base']
            : [];

        const viriyaLink = base.find(
            v => typeof v === 'string' && v.startsWith(`[[${VIRIYA_MD_PATH}`)
        ) as string | undefined;

        if (!viriyaLink) {
            new Notice('viriya에 등록된 파일이 아닙니다.');
            return;
        }

        try {
            const copiedFile = vault.getAbstractFileByPath(`${VIRIYA_MD_PATH}/${activeFile.name}`);
            ...
            await this.plugin.app.fileManager.processFrontMatter(activeFile, (frontmatter) => {
                ...
                frontmatter['base'] = base.filter(
                    (v: unknown) => !(typeof v === 'string' && v.startsWith(`[[${VIRIYA_MD_PATH}`))
                );
            });

            new Notice(`${activeFile.basename}을 viriya에서 내렸습니다.`);
        } catch (e) {
            new Notice(e instanceof Error ? e.message : '오류가 발생했습니다.');
        }
    }
