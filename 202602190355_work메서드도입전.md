...
import {
    ...
    prepareFuzzySearch, SuggestModal, PluginSettingTab, Setting, FileView
} from 'obsidian';
...
export default class ATOZVER6Plugin extends Plugin {
    ...
    async onload() {
        ...
        this.app.workspace.onLayoutReady(() => {
            if (this.settings.workCleanupOnStartup) {
                this.runWorkStartupSequence();
            }
        });
    }
    ...
    registerCommands() {
        ...
        // [Properties]
        this.addCommand({ id: "insert-properties", name: "속성 삽입", icon: "help", editorCallback: (editor: Editor) => this.handleInsertProperties(editor) });
        ...
        // [Work]
        this.addCommand({ id: 'open-work-file', name: '작업 문서 열기', callback: async () => { await this.cleanWorkAndBackup(); await this.cleanupTabs(); await this.openWorkFile(); } });
     ...
     */
    private async runWorkStartupSequence() {
        await this.cleanupTabs();
    ...
    async cleanWorkAndBackup() {
        ...
        try {
            ...
            // 현재 활성 탭이 작업 파일이라면, 에디터 내용을 가져옴 (가장 최신 데이터)
            if (activeLeaf && activeLeaf.file?.path === workPath) {
                activeEditor = activeLeaf.editor;
                content = activeEditor.getValue();
            } else {
                // 아니라면 파일에서 읽음 (자동 저장된 데이터)
                content = await vault.read(workFile);
            }

            // 내용이 비어있으면 백업 불필요, 성공으로 처리
            if (!content.trim()) return true;

            // 백업 파일 존재 여부를 먼저 확인 — 없으면 데이터 유실 방지를 위해 중단
            ...
            .addButton((btn) => 
                btn
                    ...
                    try {
                        ...
                        statusSetting.setDesc('JSON 문법 오류로 설정을 저장할 수 없습니다.');
                    }
                })
            )
            .addButton((btn) => 
